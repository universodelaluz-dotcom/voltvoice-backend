...
const normalizePlan = (value = 'free') => String(value || 'free').trim().toLowerCase().replace(/[\s-]+/g, '_');
...
const toPublicPlan = (planValue) => {
  const normalized = normalizePlan(planValue);
  const direct = BACKEND_PLAN_TO_PUBLIC_PLAN[normalized];
